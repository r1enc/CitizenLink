...
class LoginRequest extends FormRequest
{
    ...
    {
        return [
            'login' => ['required', 'string'], // Kita pakai 'login' biar bisa NIK/Email/Username
    ...
    public function authenticate(): void
    {
        ...
        // 2. Deteksi Tipe Login (Otomatis)
        $fieldType = 'username'; // Default anggap username
        if (is_numeric($input)) {
            $fieldType = 'nik'; // Kalo angka semua -> NIK
        } elseif (filter_var($input, FILTER_VALIDATE_EMAIL)) {
            $fieldType = 'email'; // Kalo format email -> Email
        }
